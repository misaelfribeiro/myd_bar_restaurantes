@extends('layouts.app')
@section('title', 'Editar Empresa - ' . $empresa->nome_fantasia)
@section('content')
<div class="container-fluid">
 <div class="d-flex justify-content-between align-items-center mb-4">
 <h1><i class="fas fa-building me-2"></i>Editar Empresa</h1>
 <a href="{{ route('empresas.show', $empresa->id) }}" class="btn btn-secondary">
 <i class="fas fa-arrow-left me-2"></i>Voltar
 </a>
 </div>
 <form action="{{ route('empresas.update', $empresa->id) }}" method="POST" enctype="multipart/form-data" id="formEmpresa">
 @csrf
 @method('PUT')
 <div class="row">
 <div class="col-lg-8">
 ...
 <option value="{{ $matriz->id }}" {{ old('empresa_matriz_id', $empresa->empresa_matriz_id) == $matriz->id ? 'selected' : '' }}>
 ...
 id="descricao" name="descricao" rows="3">{{ old('descricao', $empresa->descricao) }}</textarea>
 @error('descricao')
 <div class="invalid-feedback">{{ $message }}</div>
 @enderror
 </div>
 ...
 <option value="{{ $uf }}" {{ old('endereco_estado', $empresa->endereco_estado) == $uf ? 'selected' : '' }}>{{ $uf }}</option>
 @endforeach
 </select>
 ...
 value="{{ old('latitude', $empresa->latitude) }}" readonly>
 @error('latitude')
 <div class="invalid-feedback">{{ $message }}</div>
 @enderror
 </div>
 ...
 value="{{ old('longitude', $empresa->longitude) }}" readonly>
 @error('longitude')
 <div class="invalid-feedback">{{ $message }}</div>
 @enderror
 </div>
 ...
 value="{{ old('horario_abertura', $empresa->horario_abertura) }}">
 ...
 value="{{ old('horario_fechamento', $empresa->horario_fechamento) }}">
 ...
 id="dia_{{ $dia }}"
 {{ (is_array(old('dias_funcionamento', $empresa->dias_funcionamento)) && in_array($dia, old('dias_funcionamento', $empresa->dias_funcionamento))) ? 'checked' : '' }}>
 ...
 <img id="logoPreview" 
 src="{{ $empresa->logo ? asset('storage/' . $empresa->logo) : 'https://via.placeholder.com/300x300?text=Logo' }}" 
 ...
 {{ old('aceita_delivery', $empresa->aceita_delivery) ? 'checked' : '' }}>
 ...
 value="{{ old('raio_entrega_km', $empresa->raio_entrega_km) }}">
 ...
 {{ old('tipo_recebimento_pagamento', $empresa->tipo_recebimento_pagamento ?? 'manual') == 'manual' ? 'checked' : '' }}>
 ...
 {{ old('tipo_recebimento_pagamento', $empresa->tipo_recebimento_pagamento) == 'automatico' ? 'checked' : '' }}>
