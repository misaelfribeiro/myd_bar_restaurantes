@extends('layouts.app')
@section('title', 'Dashboard Master - EatsFood')
...
<div class="container-fluid px-4" style="background: #f5f7fa; min-height: 100vh; padding-top: 20px;">
 ...
 <h2 class="mb-0">{{ $stats['total_empresas'] ?? 0 }}</h2>
 <small><i class="fas fa-check-circle me-1"></i>{{ $stats['empresas_ativas'] ?? 0 }} ativas</small>
 </div>
 ...
 <h2 class="mb-0">R$ {{ number_format($stats['receita_mensalidades'] ?? 0, 0, ',', '.') }}</h2>
 ...
 <h2 class="mb-0">{{ number_format($stats['pedidos_mes'] ?? 0, 0, ',', '.') }}</h2>
 <small><i class="fas fa-calendar me-1"></i>{{ $stats['pedidos_hoje'] ?? 0 }} hoje</small>
 </div>
 <div class="bg-white bg-opacity-25 p-3 rounded">
 ...
 <h2 class="mb-0">{{ $stats['total_usuarios_operacionais'] ?? 0 }}</h2>
 <small><i class="fas fa-user-check me-1"></i>{{ $stats['usuarios_ativos_hoje'] ?? 0 }} ativos hoje</small>
 </div>
 <div class="bg-white bg-opacity-25 p-3 rounded">
 ...
 <h2 class="mb-0">{{ number_format($stats['total_clientes'] ?? 0, 0, ',', '.') }}</h2>
 <small class="text-success">
 <i class="fas fa-arrow-up me-1"></i>{{ $stats['clientes_novos_mes'] ?? 0 }} novos este mês
 </small>
 </div>
 ...
 <h2 class="mb-0">{{ $stats['total_entregadores'] ?? 0 }}</h2>
 <small class="text-success">
 <i class="fas fa-check-circle me-1"></i>{{ $stats['entregadores_ativos'] ?? 0 }} ativos
 </small>
 </div>
 <div class="bg-white bg-opacity-50 p-3 rounded">
 ...
 @if(isset($stats['entregadores_pendentes']) && $stats['entregadores_pendentes'] > 0)
 <div class="mt-2">
 <a href="{{ route('entregadores.index', ['status' => 'pendente']) }}" class="btn btn-sm btn-warning w-100">
 <i class="fas fa-clock me-1"></i>{{ $stats['entregadores_pendentes'] }} Pendentes de Aprovação
 </a>
 </div>
 @endif
 </div>
 ...
 <h2 class="mb-0">R$ {{ $stats['pedidos_mes'] > 0 ? number_format($stats['valor_total_mes'] / $stats['pedidos_mes'], 2, ',', '.') : '0,00' }}</h2>
 ...
 <h2 class="mb-0">R$ {{ number_format($stats['valor_total_mes'], 0, ',', '.') }}</h2>
 <small class="text-muted">
 ...
 <div>
 <h3 class="mb-0">{{ $stats['empresas_trial'] }}</h3>
 ...
 <div>
 <h3 class="mb-0">{{ $stats['empresas_suspensas'] }}</h3>
 ...
 <div>
 <h3 class="mb-0">{{ $stats['novas_empresas_mes'] }}</h3>
 ...
 <div>
 <h3 class="mb-0">R$ {{ number_format($stats['taxa_mes_estimada'], 0, ',', '.') }}</h3>
 ...
 <span class="badge 
 @if($plano->plano == 'enterprise') bg-primary
 @elseif($plano->plano == 'premium') bg-success
 @elseif($plano->plano == 'profissional') bg-info
 @else bg-secondary
 @endif">
 {{ strtoupper($plano->plano) }}
 </span>
 <strong>{{ $plano->total }}</strong>
 </div>
 ...
 @else bg-secondary
 @endif" 
 style="width: {{ ($plano->total / $stats['total_empresas']) * 100 }}%">
 </div>
 </div>
 </div>
 @endforeach
 ...
 <small class="text-muted">{{ $empresa->tenant_code }}</small>
 </td>
 <td class="text-center">
 <span class="badge bg-primary">{{ $empresa->total_pedidos }}</span>
 </td>
 <td class="text-end">
 <strong class="text-success">R$ {{ number_format($empresa->valor_total, 2, ',', '.') }}</strong>
 </td>
 </tr>
 @empty
 <tr>
 ...
 <td>{{ \Carbon\Carbon::parse($empresa->data_fim_contrato)->format('d/m/Y') }}</td>
 <td>
 <span class="badge 
 @if($empresa->dias_restantes_contrato <= 7) bg-danger
 @elseif($empresa->dias_restantes_contrato <= 15) bg-warning
 @else bg-info
 @endif">
 {{ $empresa->dias_restantes_contrato }} dias
 </span>
 </td>
 <td class="text-end">R$ {{ number_format($empresa->valor_mensalidade, 2, ',', '.') }}</td>
 <td class="text-center">
 <a href="{{ route('admin.tenants.show', $empresa->id) }}" class="btn btn-sm btn-primary">
 ...
 <div class="row g-2">
 <div class="col-md-3">
 <a href="{{ route('admin.tenants.list') }}" class="btn btn-outline-primary w-100">
 <i class="fas fa-list me-2"></i>Listar Todas Empresas
 </a>
 </div>
 <div class="col-md-3">
 <a href="{{ route('entregadores.index', ['status' => 'pendente']) }}" class="btn btn-outline-warning w-100">
 <i class="fas fa-user-clock me-2"></i>Aprovar Entregadores
 @if(isset($stats['entregadores_pendentes']) && $stats['entregadores_pendentes'] > 0)
 <span class="badge bg-danger ms-1">{{ $stats['entregadores_pendentes'] }}</span>
 @endif
 </a>
 </div>
 <div class="col-md-3">
 <a href="{{ route('admin.tenants.financial') }}" class="btn btn-outline-success w-100">
 <i class="fas fa-chart-line me-2"></i>Relatório Financeiro
 </a>
 </div>
 <div class="col-md-3">
 <a href="{{ route('empresas.create') }}" class="btn btn-outline-info w-100">
 ...
 <div class="col-md-3">
 <a href="{{ route('admin.carla') }}" class="btn btn-outline-purple w-100" style="border-color: #667eea; color: #667eea;">
 ...
 <a href="{{ route('admin.carla') }}" class="btn btn-primary">
