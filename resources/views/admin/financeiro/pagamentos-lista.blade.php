...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ route('admin.financeiro.pagamentos.dashboard') }}" class="btn btn-secondary">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.financeiro.pagamentos.lista') }}">
                <div class="row">
                    <div class="col-md-3">
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <select name="metodo" class="form-control">
                                ...
                                <option value="credit_card" {{ request('metodo') == 'credit_card' ? 'selected' : '' }}>Crédito</option>
                                <option value="debit_card" {{ request('metodo') == 'debit_card' ? 'selected' : '' }}>Débito</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <a href="{{ route('admin.financeiro.pagamentos.lista') }}" class="btn btn-secondary">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            ...
            <span class="badge badge-info">{{ $pagamentos->total() }} registros</span>
        </div>
        <div class="card-body">
                ...
                <table class="table table-sm table-bordered table-hover" style="font-size: 0.85rem; min-width: 100%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="#" data-toggle="modal" data-target="#detalhesModal{{ $pag->id }}">
                                    #{{ $pag->numero_pedido }}
                                </a>
                            </td>
                            <td>
                                <strong>{{ $pag->tenant_code }}</strong>
                                @if($pag->pedido && $pag->pedido->empresa)
                                <br><small class="text-muted" style="font-size: 0.75rem;">{{ Str::limit($pag->pedido->empresa->nome, 15) }}</small>
                                @endif
                            </td>
                            <td>
                                @if($pag->payment_method == 'pix')
                                <span class="badge badge-primary">PIX</span>
                                @elseif($pag->payment_method == 'credit_card')
                                <span class="badge badge-success">Crédito</span>
                                @elseif($pag->payment_method == 'debit_card')
                                <span class="badge badge-info">Débito</span>
                                @else
                                {{ $pag->payment_method }}
                                @endif
                            </td>
                            <td>R$ {{ number_format($pag->amount, 2, ',', '.') }}</td>
                            <td>
                                @php
                                ...
                                @if($totalEstornos > 0)
                                    ...
                                    <br><span class="badge badge-warning badge-sm mt-1">{{ $estornosPendentes }} pend.</span>
                                    @endif
                                @elseif($estornosPendentes > 0)
                                    <span class="badge badge-warning badge-sm">{{ $estornosPendentes }} pend.</span>
                                @else
                                    <span class="text-muted">-</span>
                                @endif
                            </td>
                            <td class="text-info">R$ {{ number_format($pag->platform_fee, 2, ',', '.') }}</td>
                            <td class="text-success">
                                @php
                                    $liquidoReal = $pag->net_amount - $totalEstornos;
                                @endphp
                                <strong>R$ {{ number_format($liquidoReal, 2, ',', '.') }}</strong>
                            </td>
                            <td>
                                @if($pag->status == 'pending')
                                <span class="badge badge-warning">Pendente</span>
                                @elseif($pag->status == 'approved')
                                <span class="badge badge-success">Aprovado</span>
                                @elseif($pag->status == 'rejected')
                                <span class="badge badge-danger">Rejeitado</span>
                                @elseif($pag->status == 'cancelled')
                                <span class="badge badge-secondary">Cancelado</span>
                                @elseif($pag->status == 'refunded')
                                <span class="badge badge-dark">Estornado</span>
                                @endif
                            </td>
                            <td>
                                <button class="btn btn-sm btn-info" data-toggle="modal" 
                                        data-target="#detalhesModal{{ $pag->id }}">
                                    <i class="fas fa-eye"></i>
                                </button>
                                @if($pag->status == 'approved')
                                <button class="btn btn-sm btn-danger" 
                                        onclick="confirmarEstorno({{ $pag->id }})">
                        ...
                        <!-- Modal Detalhes -->
                        <div class="modal fade" id="detalhesModal{{ $pag->id }}" tabindex="-1" role="dialog">
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Detalhes do Pagamento #{{ $pag->id }}</h5>
                                    ...
                                    <div class="modal-body">
                                            ...
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col">
                                                    ...
                                                    <p class="mb-0">
                                                        ...
                                                        <span class="h4 text-danger">R$ {{ number_format($totalEstornosModal, 2, ',', '.') }}</span>
                                                    </p>
                                                    <small>
                                                        Líquido após estorno: 
                                                        <strong>R$ {{ number_format($pag->net_amount - $totalEstornosModal, 2, ',', '.') }}</strong>
                                                    </small>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <table class="table table-sm">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>#{{ $pag->numero_pedido }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>MP ID:</strong></td>
                                                        <td>{{ $pag->mp_payment_id }}</td>
                                                    </tr>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <span class="badge badge-dark">↺ Estornado</span>
                                                            @else
                                                            <span class="badge badge-secondary">{{ ucfirst($pag->status) }}</span>
                                                            @endif
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Criado em:</strong></td>
                                                        <td>{{ $pag->created_at->format('d/m/Y H:i:s') }}</td>
                                                    </tr>
                                                    @if($pag->paid_at)
                                                    <tr>
                                                        <td><strong>Pago em:</strong></td>
                                                        <td>{{ $pag->paid_at->format('d/m/Y H:i:s') }}</td>
                                                    </tr>
                                                    @endif
                                                </table>
                                            </div>
                                            <div class="col-md-6">
                                                ...
                                                <table class="table table-sm">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-warning">R$ {{ number_format($pag->delivery_fee ?? 0, 2, ',', '.') }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td><strong>Taxa Gateway:</strong></td>
                                                        <td>R$ {{ number_format($pag->gateway_fee, 2, ',', '.') }}</td>
                                                    </tr>
                                                    <tr class="bg-light">
                                                        <td><strong>Líquido Restaurante:</strong></td>
                                                        <td class="text-success">
                                                            <strong>R$ {{ number_format($pag->net_amount, 2, ',', '.') }}</strong>
                                                        </td>
                                                    </tr>
                                                </table>
                                                ...
                                                <p class="mb-0">
                                                    <strong>{{ $pag->tenant_code }}</strong><br>
                                                    @if($pag->pedido && $pag->pedido->empresa)
                                                    {{ $pag->pedido->empresa->nome }}
                                                    @endif
                                                </p>
                                            </div>
                                        </div>

                                        @if($pag->refund_reason)
                                        <hr>
                                        ...
                                        @if($pag->pedido && $pag->pedido->estornos && $pag->pedido->estornos->count() > 0)
                                        <hr>
                                        <h6 class="font-weight-bold">
                                            <i class="fas fa-undo text-danger"></i> 
                                            Estornos do Pedido ({{ $pag->pedido->estornos->count() }})
                                        </h6>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-sm table-bordered">
                                                ...
                                                <tbody>
                                                    @foreach($pag->pedido->estornos as $estorno)
                                                    <tr class="{{ $estorno->status == 'aprovado' ? 'table-danger' : ($estorno->status == 'pendente' ? 'table-warning' : '') }}">
                                                        <td>
                                                            <span class="badge badge-{{ $estorno->tipo == 'parcial' ? 'info' : 'warning' }}">
                                                        ...
                                                        <td>
                                                            <small>{{ Str::limit($estorno->motivo, 40) }}</small>
                                                        </td>
                                                        <td>
                                                            <small>
                                                                <strong>{{ $estorno->solicitante->name ?? 'N/A' }}</strong><br>
                                                                {{ $estorno->solicitado_em->format('d/m/Y H:i') }}
                                                            </small>
                                                        </td>
                                                        <td>
                                                            @if($estorno->aprovador)
                                                            <small>
                                                                <strong>{{ $estorno->aprovador->name }}</strong><br>
                                                                {{ $estorno->processado_em ? $estorno->processado_em->format('d/m/Y H:i') : '-' }}
                                                                @if($estorno->observacoes_aprovacao)
                                                                <br><em class="text-muted">"{{ Str::limit($estorno->observacoes_aprovacao, 30) }}"</em>
                                                                @endif
                                                            </small>
                                                            @else
                                                            <span class="text-muted">-</span>
                                                    ...
                                                    <tr class="table-dark">
                                                        ...
                                                        <td colspan="4">
                                                            <strong class="text-danger">
                                                                R$ {{ number_format($pag->pedido->estornos->where('status', 'aprovado')->sum('valor'), 2, ',', '.') }}
                                                            </strong>
